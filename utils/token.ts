...
import { TOKENS_ADDRESS } from "../scripts/constants";
dotenv.config();

const STARKNET_URL = process.env.RPC_ENDPOINT || "http://127.0.0.1:5050";
const PATH_TOKEN = path.resolve(
  __dirname,
  "../contracts/fun/target/dev/joy_fun_ERC20.contract_class.json"
);
const PATH_TOKEN_COMPILED = path.resolve(
  __dirname,
  "../contracts/fun/target/dev/joy_fun_ERC20.compiled_contract_class.json"
);
...
export const createToken = async () => {
  try {
    ...
    const contractConstructor: Calldata = CallData.compile({
      ...
      initial_supply: cairo.uint256(10000),
